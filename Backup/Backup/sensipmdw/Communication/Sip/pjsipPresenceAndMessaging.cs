 ...
 * 
 * @see http://sites.google.com/site/Sensipvoip
 * 
 */
...
namespace Sensip.Sip
{
  ...
  public class pjsipPresenceAndMessaging : IPresenceAndMessaging
  {
    ...
    [DllImport(PJSIP_DLL,EntryPoint="dll_setComplexStatus")]
    private static extern int dll_setComplexStatus(int accId, int presence_state, string message);
    #endregion 
    ...
    public override int setComplexStatus(int accId, EUserStatus status, string xmlInfo)
    {
      if ((!pjsipStackProxy.Instance.IsInitialized) || (accId < 0)) return -1;
      ...
      return dll_setComplexStatus(Config.Accounts[accId].Index, (int)status, xmlInfo);
    }
